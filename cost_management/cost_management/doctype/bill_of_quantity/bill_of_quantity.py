...
class BillofQuantity(SellingController):
	...
	def validate_shopping_cart_items(self):
		...
		for item in self.items:
			...
			if not has_web_item:
				frappe.throw(
					_("Row #{0}: Item {1} must have a Website Item for Shopping Cart Bill of Quantitys").format(
	...
	def get_valid_items(self):
		...
		def is_in_sales_order(row):
			in_sales_order = bool(
				frappe.db.exists(
					"Sales Order Item", {"bill_of_quantity_item": row.name, "item_code": row.item_code, "docstatus": 1}
				)
			)
	...
	def update_lead(self):
		if self.bill_of_quantity_to == "Lead" and self.party_name:
			frappe.get_doc("Lead", self.party_name).set_status(update=True)

	def set_customer_name(self):
		if self.party_name and self.bill_of_quantity_to == "Customer":
			self.customer_name = frappe.db.get_value("Customer", self.party_name, "customer_name")
		elif self.party_name and self.bill_of_quantity_to == "Lead":
	...
	def declare_enquiry_lost(self, lost_reasons_list, competitors, detailed_reason=None):
		if not (self.is_fully_ordered() or self.is_partially_ordered()):
			get_lost_reasons = frappe.get_list("Bill of Quantity Lost Reason", fields=["name"])
	...
	def on_submit(self):
		...
		self.update_opportunity("Bill of Quantity")
		self.update_lead()
...
def make_sales_order(source_name: str, target_doc=None):
	if not frappe.db.get_singles_value(
		"Selling Settings", "allow_sales_order_creation_for_expired_bill_of_quantity"
	):
		bill_of_quantity = frappe.db.get_value(
			"Bill of Quantity", source_name, ["transaction_date", "valid_till"], as_dict=1
		)
		if bill_of_quantity.valid_till and (
			bill_of_quantity.valid_till < bill_of_quantity.transaction_date or bill_of_quantity.valid_till < getdate(nowdate())
		):
			frappe.throw(_("Validity period of this bill_of_quantity has ended."))
...
def _make_sales_order(source_name, target_doc=None, ignore_permissions=False):
	...
	def can_map_row(item) -> bool:
		"""
		Row mapping from Bill of Quantity to Sales order:
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"Bill of Quantity": {"doctype": "Sales Order", "validation": {"docstatus": ["=", 1]}},
			"Bill of Quantity Item": {
				"doctype": "Sales Order Item",
				"field_map": {"parent": "prevdoc_docname", "name": "bill_of_quantity_item"},
...
def set_expired_status():
	# filter out submitted non expired bill_of_quantitys whose validity has been ended
	cond = "`tabBill of Quantity`.docstatus = 1 and `tabBill of Quantity`.status NOT IN ('Expired', 'Lost') and `tabBill of Quantity`.valid_till < %s"
	...
	so_against_quo = """
		...
		WHERE
			...
			and so_item.prevdoc_docname = `tabBill of Quantity`.name"""

	# if not exists any SO, set status as Expired
	frappe.db.multisql(
		{
			"mariadb": """UPDATE `tabBill of Quantity`  SET `tabBill of Quantity`.status = 'Expired' WHERE {cond} and not exists({so_against_quo})""".format(
				cond=cond, so_against_quo=so_against_quo
			),
			"postgres": """UPDATE `tabBill of Quantity` SET status = 'Expired' FROM `tabSales Order`, `tabSales Order Item` WHERE {cond} and not exists({so_against_quo})""".format(
				cond=cond, so_against_quo=so_against_quo
			),
		},
		(nowdate()),
...
def _make_sales_invoice(source_name, target_doc=None, ignore_permissions=False):
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"Bill of Quantity": {"doctype": "Sales Invoice", "validation": {"docstatus": ["=", 1]}},
			"Bill of Quantity Item": {
...
def _make_customer(source_name, ignore_permissions=False):
	bill_of_quantity = frappe.db.get_value(
		"Bill of Quantity", source_name, ["order_type", "party_name", "customer_name"], as_dict=1
	)

	if bill_of_quantity and bill_of_quantity.get("party_name"):
		if not frappe.db.exists("Customer", bill_of_quantity.get("party_name")):
			lead_name = bill_of_quantity.get("party_name")
			...
			if not customer_name:
				...
				if bill_of_quantity.get("party_name") == "Shopping Cart":
			...
			return frappe.get_doc("Customer", bill_of_quantity.get("party_name"))
